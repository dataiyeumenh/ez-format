...
const RetryBatchReview = ({ repair, busy, onCreate, onDownload }) => {
  ...
  const acknowledgementScope = `${repair?.version ?? ""}:${readiness.hash || ""}`;
  const [warningAcknowledgement, setWarningAcknowledgement] = useState({
    scope: acknowledgementScope,
    accepted: false,
  });
  const warningsAcknowledged = warningAcknowledgement.scope === acknowledgementScope
    && warningAcknowledgement.accepted;
  useEffect(() => {
    setWarningAcknowledgement({ scope: acknowledgementScope, accepted: false });
  }, [acknowledgementScope]);
  const readinessSummary = readiness.summary || repair?.readinessSummary || {};
  const gate = getRetryGate({
    status: repair?.status,
    backendGate: repair?.retryGate,
  ...
  return <div className="space-y-4">
    <div className={`rounded-xl border p-4 ${gate.enabled ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
      ...
      {Number(readinessSummary.warning || 0) > 0 && <label className="mt-4 flex min-h-11 items-center gap-3 text-sm font-medium text-slate-800"><input type="checkbox" checked={warningsAcknowledged} onChange={(event) => setWarningAcknowledgement({ scope: acknowledgementScope, accepted: event.target.checked })} className="h-5 w-5 rounded border-slate-300 text-primary-600" />Tôi đã rà soát {readinessSummary.warning} cảnh báo trong phiên kiểm tra này.</label>}
    </div>
