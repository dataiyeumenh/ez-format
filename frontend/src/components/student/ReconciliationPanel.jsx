  ...
  if (loading)
    return (
      <p className="rounded-3xl bg-white p-6 text-sm text-slate-600">
        Đang đối chiếu các tổng độc lập…
      </p>
    );
  if (error)
    return (
  ...
  return (
    <section>
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black text-slate-950">Reconciliation</h2>
          <p className="text-sm text-slate-500">
            Kết quả deterministic; lý do bên dưới chỉ là giả thuyết.
          </p>
        </div>
        <button
          type="button"
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 font-black text-slate-950">{state.label}</p>
                </div>
                <Icon
                  className={
                    state.kind === "success"
              ...
              {item.left && item.right ? (
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                  <div className="rounded-xl bg-slate-50 p-3">
                    ...
                    <strong>{item.left.value}</strong>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-3">
                    <span className="block text-xs text-slate-500">
                      {item.right.label}
                    </span>
                    <strong>{item.right.value}</strong>
                  </div>
                </div>
              ) : isReconciliationEvidenceInsufficient(item) ? (
                <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">
                  insufficient_data — không được coi là khớp.
                </p>
              ) : null}
              {item.delta !== undefined && (
                <p className="mt-3 text-sm font-black text-slate-800">
                  Chênh lệch: {item.delta} · Dung sai: {item.tolerance ?? "n/a"}
                </p>
              )}
              {(item.possibleReasonsVi || []).length > 0 && (
                <div className="mt-3 text-xs leading-5 text-slate-600">
                  <strong>Giả thuyết:</strong> {item.possibleReasonsVi.join(" · ")}
                </div>
              )}
              {item.fixHintVi && (
