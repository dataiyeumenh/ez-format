...
def healthz() -> dict[str, object]:
    ai_status = _ai_runtime_status()
    return {
        "status": "ok",
        "ai": ai_status,
        "capabilities": _converter_capabilities(ai_status),
    }
...
def _ai_runtime_status() -> str:
    ...
    if ai_provider == "remote_http":
        ...
        # Direct Ollama access is intentionally unsupported. All inference must
        # pass through the authenticated Local AI Gateway.
        ai_status = "disabled"

    return ai_status


def _converter_capabilities(ai_status: str | None = None) -> dict[str, object]:
    import os

    effective_ai_status = ai_status if ai_status is not None else _ai_runtime_status()
    return {
...
@app.post("/api/v1/mappings/session")
async def sync_misa_mapping_session(body: dict) -> JSONResponse:
    try:
        payload = await run_in_threadpool(
            sync_mapping_session,
            upload_id=str(body["upload_id"]),
...
@app.get("/api/v1/sessions/{session_id}/revisions")
...
@app.post("/api/v1/sessions/{session_id}/revisions/{revision}/activate")
...
@app.post("/api/v1/sessions/{session_id}/anomalies/detect")
...
@app.get("/api/v1/sessions/{session_id}/anomalies")
...
@app.post("/api/v1/sessions/{session_id}/anomalies/{anomaly_id}/review")
...
@app.post("/api/v1/sessions/{session_id}/corrections/propose")
...
@app.post("/api/v1/sessions/{session_id}/corrections/simulate")
...
@app.post("/api/v1/sessions/{session_id}/corrections/apply")
...
@app.post("/api/v1/sessions/{session_id}/corrections/undo")
...
@app.post("/api/v1/sessions/{session_id}/comparison-files")
...
@app.delete("/api/v1/sessions/{session_id}/comparison-files/{file_id}")
...
@app.post("/api/v1/sessions/{session_id}/reconciliation/run")
...
@app.get("/api/v1/sessions/{session_id}/reconciliation/{report_id}")
...
def _authorized_operation_store(
    session_id: str, conversion_context_token: str | None
) -> OperationStore:
    store = OperationStore()
    session = store.load_session(session_id)
    if session.owner_scope.startswith("local:"):
        if unauthenticated_local_operations_enabled():
            return store
        raise ConversionContextError(
            "Local operation session chỉ được phép khi bật dev flag",
            status_code=401,
        )
    ...
    expected_owner = conversion_context_owner_scope(claims)
    if session.owner_scope != expected_owner:
