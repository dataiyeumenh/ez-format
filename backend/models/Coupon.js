...
const couponSchema = new mongoose.Schema(
  {
    ...
    limitPerUser: {
      ...
      min: [1, "Mỗi người dùng được dùng tối thiểu 1 lần"],
      default: 1,
    },
    startDate: {
      type: Date,
...
couponSchema.pre("validate", function validateCoupon(next) {
  ...
  if (Number(this.usageCount || 0) > Number(this.usageLimit || 0)) {
    this.invalidate("usageCount", "Số lượt đã dùng không được vượt giới hạn");
  }
  next();
});
