...
function contextSecret() {
  const dedicated = String(process.env.CONVERSION_CONTEXT_SECRET || "").trim();
  ...
  const allowDevelopmentFallback =
    ...
    ["1", "true", "yes"].includes(
      String(process.env.CONVERSION_CONTEXT_ALLOW_JWT_SECRET_FALLBACK || "")
        .trim()
        .toLowerCase(),
    );
  const fallback = String(process.env.JWT_SECRET || "").trim();
  if (allowDevelopmentFallback && fallback) return fallback;
  throw new Error("CONVERSION_CONTEXT_SECRET chưa được cấu hình");
}

function assertConversionContextConfig() {
  const secret = contextSecret();
  const environment = String(process.env.NODE_ENV || "").trim().toLowerCase();
  if (
    environment === "production" &&
    secret.length < MIN_PRODUCTION_CONTEXT_SECRET_LENGTH
  ) {
    throw new Error(
      "CONVERSION_CONTEXT_SECRET must be at least 32 characters in production",
    );
  }
}
...
function createConversionContextToken({
  ...
  const normalizedWorkspaceId =
    workspaceId == null || String(workspaceId).trim() === ""
      ? null
      : String(workspaceId);
  const expectedOwnerScope = normalizedWorkspaceId
    ? `workspace:${normalizedWorkspaceId}`
    : `user:${normalizedUserId}`;
  if (ownerScope != null && String(ownerScope).trim() !== expectedOwnerScope) {
  ...
  return jwt.sign(
    {
      ...
      snapshot_set_hash:
        snapshotSetHash == null ? null : String(snapshotSetHash),
      ...
      max_file_bytes: Number(maxFileBytes) || 0,
      ai_mapping_opt_in: aiMappingOptIn === true,
...
function assertConversionContextBinding(claims, binding = {}) {
  const fields = [
  ...
  for (const [claimName, expected, label] of fields) {
    const normalizedExpected = String(expected || "").trim();
    if (!normalizedExpected || String(claims?.[claimName] || "") !== normalizedExpected) {
      throw new Error(`${label} không khớp conversion context`);
    }
  }
  if (
    binding.conversionContextId &&
    String(claims?.conversion_context_id || "") !== String(binding.conversionContextId)
  ) {
    throw new Error("Conversion context id không khớp conversion run");
  }
  if (
    binding.operationSessionId &&
    String(claims?.operation_session_id || "") !== String(binding.operationSessionId)
  ) {
    throw new Error("Operation session không khớp conversion context");
  }
  if (
    binding.userId &&
    String(claims?.user_id || "") !== String(binding.userId)
  ) {
    throw new Error("Conversion context không thuộc người dùng này");
  }
  if (
    binding.ownerScope &&
    String(claims?.owner_scope || "") !== String(binding.ownerScope)
  ) {
    throw new Error("Conversion context owner scope không hợp lệ");
  }
  if (
    binding.requiredScope &&
    (!Array.isArray(claims?.scopes) || !claims.scopes.includes(binding.requiredScope))
  ) {
    throw new Error(`Conversion context thiếu quyền ${binding.requiredScope}`);
  }
  return claims;
}
