...
const CONVERTER_SERVICE_TOKEN_PLACEHOLDER = "replace-with-a-long-random-secret";
const MIN_PRODUCTION_SERVICE_TOKEN_CHARS = 32;
...
function converterBaseUrl() {
  ...
  return validateInterServiceUrl(configured);
}

function validateInterServiceUrl(value) {
  let parsed;
  try {
    parsed = new URL(String(value || "").trim());
  } catch (cause) {
    const error = gatewayError(503, "Converter internal URL không hợp lệ", cause);
    error.code = "INVALID_CONVERTER_URL";
    throw error;
  }
  if (
    !parsed.hostname ||
    ...
    const error = gatewayError(503, "Converter internal URL không hợp lệ");
    error.code = "INVALID_CONVERTER_URL";
    throw error;
  }
  ...
  const allowInsecure =
    ...
    ["1", "true", "yes"].includes(
      String(process.env.CONVERTER_ALLOW_INSECURE_LOCALHOST || "")
        .trim()
        .toLowerCase(),
    );
  if (!isLoopbackHostname(parsed.hostname) || !allowInsecure) {
    const error = gatewayError(
      503,
      "Converter internal URL phải dùng HTTPS ngoài localhost",
    );
    error.code = "INSECURE_CONVERTER_URL";
    throw error;
  }
...
function isConverterGatewayUsageReady(env = process.env) {
  return (
    String(env.CONVERTER_PUBLIC_PROXY_ENABLED || "false")
      .trim()
      .toLowerCase() === "true" &&
    String(env.CONVERTER_GATEWAY_USAGE_READY || "false")
      .trim()
      .toLowerCase() === "true"
  );
}
...
function assertConverterGatewayStartupConfig(env = process.env) {
  ...
  if (!internalUrl) {
    const error = gatewayError(503, "CONVERTER_INTERNAL_URL chưa được cấu hình");
    error.code = "MISSING_CONVERTER_INTERNAL_URL";
    throw error;
  }
  validateInterServiceUrl(internalUrl);

  const serviceToken = String(env.CONVERTER_SERVICE_TOKEN || "").trim();
  if (!serviceToken) {
    const error = gatewayError(503, "CONVERTER_SERVICE_TOKEN chưa được cấu hình");
    error.code = "MISSING_CONVERTER_SERVICE_TOKEN";
    throw error;
  }
  if (
    isProductionEnvironment(env) &&
    (serviceToken.length < MIN_PRODUCTION_SERVICE_TOKEN_CHARS ||
      serviceToken === CONVERTER_SERVICE_TOKEN_PLACEHOLDER)
  ) {
    const error = gatewayError(
      503,
      "CONVERTER_SERVICE_TOKEN production phải dài ít nhất 32 ký tự và không dùng placeholder",
    );
...
function converterMaxOutputBytes() {
  const configured = Number(process.env.CONVERTER_MAX_OUTPUT_BYTES || 64 * 1024 * 1024);
  return Number.isSafeInteger(configured) && configured > 0
    ? Math.min(configured, 512 * 1024 * 1024)
    : 64 * 1024 * 1024;
}
...
async function readBoundedResponseBytes(response, maxBytes) {
  ...
  if (contentLength != null && contentLength !== "") {
    if (!/^\d+$/.test(contentLength)) {
      throw gatewayError(502, "Converter trả về Content-Length không hợp lệ");
    }
    const declaredBytes = Number(contentLength);
    if (!Number.isSafeInteger(declaredBytes) || declaredBytes > maxBytes) {
      await response.body?.cancel().catch(() => {});
      const error = gatewayError(413, "Converter trả về artifact quá lớn");
      error.code = "UPSTREAM_BODY_TOO_LARGE";
      throw error;
    }
  }
...
async function fetchConverter(path, options, binary = false) {
  ...
  try {
    ...
    if (binary && !isJson) {
      const data = await readBoundedResponseBytes(response, converterMaxOutputBytes());
      return { status: response.status, headers, data };
    }
