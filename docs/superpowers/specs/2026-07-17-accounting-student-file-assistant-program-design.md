...
current file, check their own work, follow a voucher through its accounting
...
- is learning financial accounting, tax, accounting information systems or MISA;
...
- Attempt history and skill progress.
...
-> Student inspects, asks, edits or submits an attempt
-> FastAPI revalidates deterministic state
-> Node stores metadata/progress, not raw workbook
...
```json
{
  ...
  "userId": "ObjectId",
  "revision": 2,
  "kind": "mapping|data_edit|voucher_review|reconciliation",
  "submittedStateHash": "sha256",
  "score": 82,
  "summary": {},
  "hintLevelUsed": 1,
  "createdAt": "ISO-8601"
...
```json
{
  "userId": "ObjectId",
  "skills": {
    "excel_mapping": { "score": 82, "evidenceCount": 12 },
    "document_classification": { "score": 70, "evidenceCount": 5 },
    "vat_reconciliation": { "score": 64, "evidenceCount": 8 },
    "misa_template_readiness": { "score": 85, "evidenceCount": 9 }
  },
  "updatedAt": "ISO-8601"
}
```
...
- MongoDB retains metadata and progress but not workbook bytes.
...
## 10. Phase 3 - Check My Work

### 10.1 Goals

- Let students submit their own mapping or edited data for evaluation.
- Provide progressive hints rather than immediate complete answers.
- Track skill evidence without treating AI confidence as a grade.
...
```text
Level 0  Result category only
Level 1  Explain the accounting/data concept
Level 2  Identify the relevant field group
Level 3  Identify the row/field containing the problem
Level 4  Show a worked example or expected result
```

Hint use is recorded and affects learning analytics, not accounting correctness.
...
Scoring weights are versioned. Past attempts retain the rubric version used.

### 10.5 Anti-cheating posture

- Complete answers are not shown before the configured hint level.
- AI cannot mark an answer correct.
- Assignment text generation is outside scope.
- Server-side expected states are not sent to the browser before reveal.

### 10.6 Acceptance criteria

- Same input, rubric and attempt state produce the same score.
- Attempt revision history is owner scoped.
- Hint progression works without exposing future levels.
- Progress only changes from completed deterministic evaluations.
- Students can review errors and evidence from previous attempts.
...
- Generate progressive hints from approved concepts and evidence.

### 15.2 Forbidden

- Reading another user's session.
- Creating values absent from evidence.
- Changing rule severity or score.
- Deciding that a tax treatment is legally correct without a deterministic rule.
- Revealing complete assignment answers before the configured hint level.
...
Release D  Phase 3 Check My Work and progress
...
- Citation validation.
- Attempt scoring and rubric versions.
...
- Submit an attempt and reveal hints in order.
...
- student attempts and progress are reproducible;
