...
def test_reconstruction_rejects_external_links_before_persisting_state(
    ...
    token = _token("external-run")

    with raw.open("rb") as handle:
        response = client.post(
            "/api/v1/reconstructions/analyze",
            headers=_reconstruction_headers(token),
...
def _reconstruction_headers(token: str, **extra: str) -> dict[str, str]:
    return {"X-Reconstruction-Context": token, **extra}
...
def test_reconstruction_ai_is_not_called_without_explicit_opt_in(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.ai_reconstruction_client.request_reconstruction_suggestion",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("AI must be explicit")),
    )
    raw = _write_purchase(tmp_path / "no-auto-ai.xlsx")
